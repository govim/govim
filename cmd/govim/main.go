...
func (g *govimplugin) Init(gg govim.Govim, errCh chan error) error {
	...
	g.DefineAutoCommand("", govim.Events{govim.EventBufUnload}, govim.Patterns{"*.go", "*.mod"}, false, g.vimstate.bufUnload, "eval(expand('<abuf>'))")
	g.DefineAutoCommand("", govim.Events{govim.EventBufRead, govim.EventBufNewFile}, govim.Patterns{"*.go", "*.mod"}, false, g.vimstate.bufReadPost, exprAutocmdCurrBufInfo)
	g.DefineAutoCommand("", govim.Events{govim.EventBufWritePre}, govim.Patterns{"*.go", "*.mod"}, false, g.vimstate.formatCurrentBuffer, "eval(expand('<abuf>'))")
	g.DefineAutoCommand("", govim.Events{govim.EventBufWritePost}, govim.Patterns{"*.go", "*.mod"}, false, g.vimstate.bufWritePost, "eval(expand('<abuf>'))")
	...
	g.DefineAutoCommand("", govim.Events{govim.EventBufDelete}, govim.Patterns{"*.go", "*.mod"}, false, g.vimstate.deleteCurrentBuffer, "eval(expand('<abuf>'))")
	...
	g.DefineAutoCommand("", govim.Events{govim.EventCompleteDone}, govim.Patterns{"*.go", "*.mod"}, false, g.vimstate.completeDone, "eval(expand('<abuf>'))", "v:completed_item")
	g.defineHighlights()

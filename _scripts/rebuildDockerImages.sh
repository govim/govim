...
do
	docker build --build-arg "GOBINPATH=$gobinPath" --build-arg "VBASHPATH=$vbashPath" --build-arg "GO_VERSION=$GO_VERSION" --build-arg "VIM_VERSION=$i" -t myitcv/govim:${GO_VERSION}_${i} .
done
